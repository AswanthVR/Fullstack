...
function Plans() {
    ...
    const backendUrl = "http://localhost:8181/ezpay/api/v1/recharge";
    const bearerToken = localStorage.getItem('token'); 
    const headers = {
    'Authorization': `Bearer ${bearerToken}`
    };
    ...
    const  handlePaymentSuccess = (res ,amount , rechargeId) => {
        ...
        const backendUrl = "http://localhost:8181/ezpay/api/v1/payment/make-payment";
        const bearerToken = localStorage.getItem('token'); 
        const headers = {
        'Authorization': `Bearer ${bearerToken}`
        };
    ...
    const filteredPlans = selectedCategory ? plans.filter(plan => plan.planType.toLowerCase() === selectedCategory.toLowerCase() && plan.planCategory=== "prepaid") : plans.filter(plan=> plan.planCategory === "prepaid");
    const clearFilter = () => {
        setSelectedCategory('');
    }
                    ...
                    <div className='flex md:flex-col'>
                        {planCategories.map((category, index) => (
                            ...
                            className={`text-sm md:text-base flex p-1 px-2
                                bg-gray-200 hover:bg-s4 text-black w-full md:w-4/6 border-1 border-2 border-white border-b-2 md:py-3 md:pl-4 ${selectedCategory === category.value && 'bg-s4'}`}>
                                {category.label}
                            </button>
                        ))}
                    </div>
