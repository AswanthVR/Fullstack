...
import loaddots from '../../assets/Loader/load-dot.gif'
...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate password - Add your own validation logic here
    if (password.length=="") {
      SetMsgclr('bg-red-500');
...
instantly & all with a tap!
    ...
    <div className='md:flex md:flex-col md:w-1/2 md:h-full bg-white md:rounded-r-[50px] md:rounded-none 
     sm:rounded-2xl w-full shadow-2xl md:p-[10px] lg:p-[20px] pb-[30px] justify-between  rounded-3xl' >
           ...
           <h1 className='md:hidden mt-10 block text-3xl font-bold text-teal-400 text-center'>EZpay</h1>
      {
        !user ?(loading ?(
          ...
          <div className='h-full flex justify-center items-center'>
            <img width={100} height={500} src={loaddots} alt="" />
          </div>
          </>
        ):(
          ...
          <div>
          <label className='text-teal-700 font-semibold md:text-sm lg:text-lg'  htmlFor="phone">Enter Email</label>
          <br/><input type="text"
          value={email}
          ...
          <div className=''>
          <label className='text-teal-700 font-semibold md:text-sm lg:text-lg'  htmlFor="phone">Enter Password</label>
          <input type="text"
          value={password}
          ...
          {!loading1?'Login':<TbLoader3 className =' text-white text-4xl text-center w-full animate-spin' />}
        ...
        (
          ...
          onClick={verifyOtp}
          className='bg-s3 w-36 h-10 rounded-3xl text-center text-secondary  lg-w-auto' > 
          {!loading1?'verify':<TbLoader3 className =' text-white text-4xl text-center w-full animate-spin' />} 
          </button>
